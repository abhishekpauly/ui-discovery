...
| --- | --- | --- | --- |
| `sprint/8-redaction` | `0039e98` | 5 | **Live.** `G5` merged via PR #71. `G6`/`G7` outstanding. |
...
| `G6` Redact the people out of the screenshots | P0 | 🚧 `feat/g6-redact-screenshots` |
| `G7` Egress ledger | P1 | 📋 |

## Next actions, in order

1. **`G6`** — the P0 half still open. `G5` cleaned the model and left the
   pictures untouched, and a capture of an authenticated portal is mostly
   pictures.
2. **`G7`** — the egress ledger, a rollup of network observation already
   performed.
3. **`sprint/8-redaction` → `main`** (merge commit), then tag — `RELEASING.md`.
...
- **`G5` merged as a merge commit, not a squash.** `BRANCHING.md` asks for a
  ...
  `sprint/8-redaction` carries `G5` as three commits rather than one.
